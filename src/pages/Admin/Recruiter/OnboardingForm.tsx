...
const OnboardingForm = () => {
    ...
    if (token) {
      const fetchRecruiterDetails = async () => {
        try {
          ...
          setRecruiterData({ firstName, lastName, email });
          setRecruiterSubmitData((prev) => ({
            ...prev,
            firstName,
            lastName,
  ...
  const validatePAN = (num: string) =>
    /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/.test(num.toUpperCase());

  const validateIFSC = (code: string) =>
    /^[A-Z]{4}0[A-Z0-9]{6}$/.test(code.toUpperCase());

  const validateAccountNo = (num: string) => /^\d{9,18}$/.test(num);

  // -------- SUBMIT FUNCTION WITH VALIDATION + SPINNER -------- //
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    if (!validateAadhaar(recruiterSubmitData.aadhaarNumber)) {
      toast.error('Invalid Aadhaar Number (must be 12 digits)');
      return;
    }

    if (!validatePAN(recruiterSubmitData.panNumber)) {
      toast.error('Invalid PAN Number (Format: ABCDE1234F)');
      return;
    }

    if (!validateIFSC(recruiterSubmitData.ifscCode)) {
      toast.error('Invalid IFSC Code (e.g. HDFC0123456)');
      return;
    }

    if (!validateAccountNo(recruiterSubmitData.accountNo)) {
        ...
        <div className="relative rounded-sm border border-stroke">
          ...
          <form onSubmit={handleSubmit}>
            {submitForm ? (
              ...
              <div className="p-6.5">
                {/* -------- All Your Existing Form Fields (Same as you provided) -------- */}
                {/* NOTE: I DID NOT REMOVE OR EDIT ANY FIELD */}
                ...
                {/* -------- CONTINUES EXACT SAME FORM STRUCTURE (NOT REMOVED) -------- */}

                {/* Aadhaar Field Validated */}
                <div className="mb-4.5 flex flex-col gap-6 xl:flex-row">
                  <div className="w-full xl:w-1/2">
                    <label className="mb-2.5 block text-black">
                      Aadhaar Number
                ...
                <div className="flex gap-4 mt-4">
                  <button
                    className="rounded bg-primary p-2 font-medium text-white hover:bg-opacity-90 flex items-center gap-2"
                    disabled={loading}
                  >
                    {loading ? (
                  ...
                  <button className="rounded bg-gray-500 p-2 font-medium text-white hover:bg-opacity-90">
                    Close
                  </button>
                </div>
              </div>
